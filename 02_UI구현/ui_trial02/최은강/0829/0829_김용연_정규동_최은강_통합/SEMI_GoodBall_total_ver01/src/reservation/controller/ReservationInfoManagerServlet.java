 ...
 */
@WebServlet("/reservationInfoManager.me")
public class ReservationInfoManagerServlet extends HttpServlet {
   private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ReservationInfoManagerServlet() {
        super();
...
//    String query = "CREATE OR REPLACE VIEW RESERVATIONLIST2 AS SELECT ROWNUM RNUM, DESCRESERVATION.* FROM (SELECT * FROM RESERVATION JOIN STADIUM ON (RESERVATION_STADIUM_NUM = STADIUM_NUM) JOIN BRANCH ON (RESERVATION_BRANCH_NUM = BRANCH_NUM) WHERE RESERVATION_STATUS = 'Y'";
      String query = "CREATE OR REPLACE VIEW RESERVATIONLIST2 AS SELECT ROWNUM RNUM, DESCRESERVATION.* FROM (SELECT * FROM RESERVATION JOIN STADIUM ON (RESERVATION_STADIUM_NUM = STADIUM_NUM) JOIN BRANCH ON (RESERVATION_BRANCH_NUM = BRANCH_NUM)";
      String query2 = " WHERE BRANCH_MANAGER_EMAIL=" + "'" + userId + "'";
      String query3 = " ORDER BY RESERVATION_NUM DESC) DESCRESERVATION";
      
      String query4 = query + query2 + query3;
      ...
      listCount = new ReservationService().getReservationCount2(userId);
      ...
      new ReservationService().createReservationView2(query4);
      
      pi = new PageInfo(currentPage, listCount, pageLimit, boardLimit, maxPage, startPage, endPage);
      ArrayList<ReservationInfo> reservationArr = new ReservationService().selectReservationList2(pi);
      ...
      for(int i = 0; i < reservationArr.size(); i++) {
         ...
         userObj.put("branch_manager_email", reservationArr.get(i).getBranch_manager_email());
